...
class Memcache extends SandGrain {
  ...
  set(key, value, expiry, callback) {
    if (_.isFunction(expiry)) {
      callback = expiry;
      expiry = this.config.defaultTTL;
    }
  ...
  add(key, val, expiry, callback) {
    if (_.isFunction(expiry)) {
      callback = expiry;
      expiry = this.config.defaultTTL;
    }
    return runPromiseOrCallback.call(this, add, callback);

    function add(callback) {
      this.client.add(key, val, expiry, callback);
    }
  }
}
